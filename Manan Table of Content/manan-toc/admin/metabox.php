...
function manan_toc_metabox_callback($post) {
    ...
    $saved = get_post_meta($post->ID, '_manan_toc_headings', true) ?: [];
    ...
    echo '<p>Select headings to include in TOC:</p>';
    foreach ($headings as $heading) {
        $checked = in_array($heading, $saved) ? 'checked' : '';
        echo '<label style="display:block;"><input type="checkbox" name="manan_toc_headings[]" value="'.esc_attr($heading).'" '.$checked.'> '.esc_html($heading).'</label>';
    }
